...
@section('content')
<div class="mu-title">
	<span class="mu-subtitle">Edit Units</span>
</div>
<div class="wrap-login100 p-t-12 col-md-offset-4">
	@foreach($units as $unit)
	<form class="login100-form validate-form"  action="{{ route('uunit', $unit->id) }}" method = "POST">
		{{ csrf_field() }}
		{{ method_field('PATCH')}}
		<div class="wrap-input100 validate-input m-b-10" data-validate = "Fullname is required">
			<input class="input100" type="text" name="unitname" placeholder="unitname" value="{{ $unit->unit_name }}">
